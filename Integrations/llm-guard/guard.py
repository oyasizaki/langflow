...
def generate_response(prompt):
    ...
    sanitized_response_text, results_valid, results_score = scan_output(
        output_scanners, prompt, response["result"]["text"]
    )
    if any(results_valid.values()) is False:
        logging.warning(f"Warning: The generated response may have security vulnerabilities. Please review the output.")
        return "Sorry, there was a problem finding an answer for you."
    
    logging.info(f"answer: {sanitized_response_text}")
